 ...
 * 先端だけ見るときは `-1` を付ける。
 * pull_request の merge commit 主語（Merge …）は lint 免除なので、
 * CI は `--tip <pr-head-sha>` で head を指す。
 *
...
export function parseCommitLogRequest(argv = []) {
	...
	if (extra.length === 0 || (extra.length === 1 && extra[0] === 'HEAD')) {
		return { ok: true, args: ['log', '-1', '--format=%s'], mode: 'tip' };
	}
	if (extra[0] === '--tip') {
		if (extra.length === 1) {
			return { ok: true, args: ['log', '-1', '--format=%s'], mode: 'tip' };
		...
		if (extra.length === 2 && isRev(extra[1])) {
			return { ok: true, args: ['log', '-1', '--format=%s', extra[1]], mode: 'tip' };
		}
		return { ok: false, reason: '--tip には revision 1件だけを付ける。' };
	}
	if (extra.length === 1 && isRange(extra[0])) {
		return { ok: true, args: ['log', extra[0], '--format=%s'], mode: 'range' };
	}
