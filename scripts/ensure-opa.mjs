#!/usr/bin/env node
/**
 * Resolve a pinned OPA binary. No PATH / no unverified OPA_BIN / no null-digest cache.
 */
...
function archKey() {
	if (process.platform !== 'linux') {
		throw new Error(`[ensure-opa] linux-only (got ${process.platform})`);
	}
	if (process.arch === 'x64') return 'linux_amd64';
	throw new Error(`[ensure-opa] unsupported arch ${process.arch}; no pinned digest`);
}
...
export function ensureOpa() {
	...
	if (!expected) throw new Error(`[ensure-opa] no pinned digest for ${key}`);
	if (process.env.OPA_BIN) {
		throw new Error('[ensure-opa] OPA_BIN is not accepted; use the pinned .tools/opa download');
	}
