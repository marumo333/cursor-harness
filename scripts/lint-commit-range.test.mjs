...
test('--tip に PR head SHA を付けたらその1件だけ', () => {
	assert.deepEqual(commitLogArgs(['--tip', 'abc1234def']), ['log', '-1', '--format=%s', 'abc1234def']);
	const r = parseCommitLogRequest(['--tip', 'abc1234def']);
	assert.equal(r.ok, true);
	assert.equal(r.mode, 'tip');
});

test('Actions の merge commit 主語は lint を常に通るので CI の tip にしてはいけない', () => {
	assert.equal(lintCommitMessage('Merge abcdef0 into 1234567').ok, true);
	assert.equal(lintCommitMessage('Merge pull request #12 from foo/bar').ok, true);
});
