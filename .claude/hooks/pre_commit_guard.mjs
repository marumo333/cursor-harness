#!/usr/bin/env node
// PreToolUse(Bash): git commit 前に 規約6 — 型チェック緑 + 秘密スキャンを強制。
...
if (existsSync('node_modules') && existsSync('package.json') && existsSync('src')) {
	try {
		execSync('pnpm run check', { stdio: 'pipe', encoding: 'utf8' });
	} catch (e) {
		console.error(
			'[pre_commit_guard] 規約6: 型チェック(pnpm run check)が失敗。緑にしてから commit してください。\n' +
