...
- **席割当の整合（[[0033]] / [[0037]] / [[0040]]）**: `model-routing.yaml` の chat_orchestrator=Grok（世代ピンは 0040）・grok_task・opus_gates・
  review_trio(Opus/Grok/Muse)・budget_guards が AGENTS / skills / `.claude/agents/*.md` 先頭事項と矛盾していないか。
- **ハーネス制約（[[0039]] / [[0046]]）**: 席は親 Grok + Opus ゲート + Muse は3体。正本は Feature。ゲートは OPA。グラフは skill/feature/cycle。
