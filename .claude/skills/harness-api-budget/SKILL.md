...
| Task **Grok**        | 明文化済みの実装 fan-out / best-of-n 試行                            |
| Task **GPT-5.6 Sol** | **高リスク3体の第3レンズのみ**（secret）。他では使わない             |

製品 builder（auth-billing / ui / db / e2e）は template に置かない。
...
`plan-confirm`(Opus Task, fan-out 時) → `parallel-dispatch` → `adversarial-review` → `verify` → `reflect`。
